...
```{r}
gdpdata |> summarize(correlation = cor(DJIA, GDP, method="pearson"))
```
...
gdpdata |> 
  ...
  labs(title = "Relationship between the DJIA and GDP in the U.S. from 2013-2024", x = "DJIA", y = "GDP")
```
...
There is a very strong correlation between the two variables. The scatterplot shows that as the DJIA increases, so does the GDP and these variables follow the trend line very closely. Therefore, the DJIA and the GDP are strongly correlated as they have a positive relationship.
